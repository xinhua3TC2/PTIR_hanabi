...
class Action(object):
    ...
    TYPES = [PLAY, DISCARD, HINT]#行动类型有三种：出牌，弃牌，给线索
    HINT_TYPES = [COLOR, NUMBER]#线索的内容包括颜色和数字
...
class HintAction(Action):
    ...
    def __init__(self, player_id, color=None, number=None, hint_type=None, value=None):
        ...
        if color is not None or number is not None:
            assert color is not None and number is None or color is None and number is not None#assert 会在后续条件不满足时报错
